...
def generate_pdf():
    excel_file = excel_path_label.cget("text")  # Obtenha o caminho do arquivo Excel
    save_path = save_path_label.cget("text")    # Obtenha o local de salvamento
    if excel_file and save_path:
        try:
            criar_pdf(excel_file, os.path.join(save_path, 'Proposta_Comercial.pdf'))
            messagebox.showinfo('Sucesso', 'Seu PDF foi gerado com sucesso! Confira o local de salvamento para ver seu PDF')
        except Exception as e:
            messagebox.showerror('Erro', 'Falha ao gerar PDF. Certifique-se de que o arquivo está no diretório correto')
    else:
        print("Por favor, selecione o arquivo Excel e o local de salvamento.")
